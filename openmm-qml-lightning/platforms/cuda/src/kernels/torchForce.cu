extern "C" __global__
void copyInputs(real* __restrict__ posTensor, real* __restrict__ boxTensor, real* __restrict__ invboxTensor, const real4* __restrict__ posq,
		int* __restrict__ atomIndex, int numAtoms, real4 periodicBoxVecX, real4 periodicBoxVecY, real4 periodicBoxVecZ, bool periodic) {
	...
	if (blockIdx.x == 0 && threadIdx.x == 0 && periodic) {
		...
		//assume cubic box for now

		invboxTensor[0] = 1.0 / periodicBoxVecX.x;
		invboxTensor[4] = 1.0 / periodicBoxVecY.y;
		invboxTensor[8] = 1.0 / periodicBoxVecZ.z;
